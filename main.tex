...
\textbf{Limitations.} The current implementation does not handle node failures
mid-query: if an intermediate node crashes, the parent will block indefinitely
waiting for \texttt{source\_done} from that child. A production system would
add per-child timeouts and partial-result delivery. Additionally, the
\texttt{hash(unique\_key) \% 9} sharding assumes uniform key distribution;
if a specific key range is heavily queried, one shard becomes a hot spot.
